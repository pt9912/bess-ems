...
| RM-M2-OP-OPEN-02 | Wie stark wird `IScheduleOptimizer` mit `MarketCommitment`-Strafkosten und Tarifmodell (LH-MKT-008) gekoppelt? | M2 minimal: Day-Ahead-Energiekosten als Objective. RM-M2-04 (Configurable Objective) erweitert das um zwei neue Komponenten (`degradation_cost` linear; `soc_target_penalty` mit Slack-Variablen) und damit zwei verschiedene LP-Patterns. **Marktverpflichtungs-Strafkosten** bleiben dort *nicht* drin: sie hängen am `MarketCommitment`-Plumbing aus RM-M2-01 (Commitment-Lookup im Horizont, Strafsatz am Commitment-Objekt). **Reserveverletzung** bleibt eigene Welle: braucht ein Reserve-Domain-Modell (FCR/aFRR/mFRR-Produkttyp, Bidirektionalität), das im Repo heute nicht existiert. **Peak-Shaving** ist LP-modellierbar (Hilfsvariable mit N Constraints), braucht aber operator-seitige Klärung der Peak-Definition (Import-only vs. Netto, Bezugszeitraum vs. Horizon, statisch vs. gestaffelt) und ist eigener Slice. |
