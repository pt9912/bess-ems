...
| API             | `POST /markets/day-ahead/optimize` löst Run aus, gibt RunId/Status/Horizon/erzeugte Fahrplanversion zurück; `GET /optimization/runs/{id}` liefert den persistierten Run-Status | Driving Adapter | LH-API-005            |
...
| ✅     | RM-M2-OP-07 | API: `POST /markets/day-ahead/optimize` + `GET /optimization/runs/{id}` | RM-M2-OP-03, RM-M1-15 | Endpoint operator-policy-guarded (analog `/operator/stop`). Validation-Reihenfolge: 400 missing-or-invalid-field → 404 asset-not-registered → 400 unknown-schedule-type → 400 invalid-request (Konstruktor-Invarianten). `OptimizationResponse` rendert `Status` als Enum und enthält RunId/Status/Horizon/ProducedScheduleVersion/TerminationReason; der zentrale snake_case-Converter liefert `failed`/`optimal`/… auf der Wire. `GET /optimization/runs/{id}` liefert den persistierten Run-Status inklusive Horizon, ObjectiveBreakdown, Inputs und ProducedSchedule-Referenz; unbekannte RunIds liefern 404. `NoOpScheduleOptimizer` wandert von `Adapters.Optimization` nach `Application.Optimization` (analog zu NoOpBatteryTelemetrySource), damit der API-only-Test-Host ihn ohne Driven-Adapter-Ref auflöst. DI: `AddBessApplicationInMemoryStores` registriert `IOptimizationRunRepository`, `IScheduleOptimizationUseCase` und `IScheduleOptimizer` (NoOp-Default). 8 Endpoint-Tests + 3 NoOp-Tests; `make build` + `make runtime` real grün. |
...
| ✅     | RM-M2-OP-09 | Replay-/Reproduzierbarkeitstest                                    | RM-M2-OP-06        | `OrToolsScheduleOptimizerReplayTests` (4 Tests) belegt LH-OPT-009-Akzeptanz: GLOP ist im Simplex deterministisch (kein Random-Seed), daher liefert OP-05 für identische Inputs **bit-exakt identische** Ergebnisse — getestet (a) auf derselben Optimizer-Instanz zweimal hintereinander, (b) auf zwei frischen Instanzen, (c) auf einem realistischen 24-h-Day-Ahead-Profil. Verglichen werden alle deterministischen Felder (Status, TerminationCode/Detail, ObjectiveValue bit-exakt, ObjectiveBreakdown Komponente-für-Komponente, Schedule-Windows mit Start/End/TargetPowerKw bit-exakt); ausgeschlossen sind nicht-deterministische Felder per Design (RunId via `Guid.NewGuid()`, CreatedAt vom `IClock`, SolverRuntime von `solver.WallTime()`). Negativ-Test: zwei Optimizer mit unterschiedlichem `InitialSocPercent` (50 % vs 15 %) liefern unterschiedliche ObjectiveValues — beweist, dass die Equality-Asserts nicht versehentlich pass-by-default sind. OPEN-03 bleibt als Soll-statt-Muss-Vermerk: für Backends ohne Determinismus (CBC mit Parallel-Cuts, gRPC-Sidecar mit Network-Order) muss diese Spec überdacht werden. |
...
| ------------ | ---------------------------------------------------------------------------------- | ----------------- |
| RM-M2-OP-OPEN-01 | HiGHS via NuGet vs. OR-Tools GLOP/CBC vs. gRPC-Sidecar — welcher Bindungspfad zuerst? | Entschieden für M2: OR-Tools GLOP als In-Process-NuGet-Adapter; gRPC-Sidecar und weitere Solver bleiben spätere Backend-Optionen hinter demselben Resultatformat. |
| RM-M2-OP-OPEN-02 | Wie stark wird `IScheduleOptimizer` mit `MarketCommitment`-Strafkosten und Tarifmodell (LH-MKT-008) gekoppelt? | M2 minimal: Day-Ahead-Energiekosten als Objective. RM-M2-04 (Configurable Objective) erweitert das um zwei neue Komponenten (`degradation_cost` linear; `soc_target_penalty` mit Slack-Variablen) und damit zwei verschiedene LP-Patterns. **Marktverpflichtungs-Strafkosten** sind als Folge-Slice *zu RM-M2-04* vorgemerkt (RM-M2-04-OPT-MARKT): die Voraussetzung — `MarketCommitment.Penalty` operativ befüllt + Commitment-Lookup für den Optimierungs-Horizont — wird von RM-M2-01 gelegt; danach ergänzt der Folge-Slice eine vierte Komponente am OR-Tools-Adapter (`commitment_deviation_penalty`) analog zum `soc_target_penalty`-Slack-Pattern. **Reserveverletzung** bleibt eigene Welle: das Reserve-Domain-Modell (FCR/aFRR/mFRR-Produkttyp, Bidirektionalität, Aktivierungsdauer) ist M4-Territorium (RM-M4-02); die LP-Strafkosten-Komponente folgt analog erst danach. **Peak-Shaving** ist LP-modellierbar (Hilfsvariable mit N Constraints), braucht aber operator-seitige Klärung der Peak-Definition (Import-only vs. Netto, Bezugszeitraum vs. Horizon, statisch vs. gestaffelt) und ist eigener Slice. |
...
| RM-M2-OP-OPEN-04 | API-Trigger: synchrone Antwort mit Solverlauf vs. asynchroner Job mit Status-Endpunkt? | M2 umgesetzt: `POST /markets/day-ahead/optimize` führt den Solver synchron aus und gibt eine Summary mit Horizon zurück; `GET /optimization/runs/{id}` liefert den persistierten Status. Ein echtes Async-Job-Modell bleibt Folgearbeit, sobald Solverlaufzeiten HTTP-Budgets überschreiten. |
| RM-M2-OP-OPEN-05 ✅ | Schedule-Replace ist nicht atomar gegen parallele Optimize-Calls auf Multi-Replica-Hosts (Review S1). | **Erledigt durch RM-M3-FUP-02** ([Plan](../done/plan-RM-M3-FUP-02.md)): `IScheduleRepository.Replace(schedule, expectedBaseVersion)` mit branched-CAS-SQL in der Dapper-Variante (Insert-Pfad `ON CONFLICT DO NOTHING`, Update-Pfad `WHERE version = @expected`); Versionskonflikt wird als `Failed`-Run mit `TerminationCode = "concurrent-version-conflict"` und `TerminationDetail = "expected=…,actual=…"` auditierbar persistiert. Per-`(asset, type)`-Semaphore bleibt erhalten als Single-Host-Optimierung; CAS deckt den Multi-Replica-Fall. |
