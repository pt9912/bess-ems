...
| RM-M3-FUP-01 (erste echte Folgemigration) | Trigger-Watch in note-RM-M3-followups.md, RM-M4-03-B als erster Konsument abgeschlossen | ✅ — `0002_regelleistung_activations.sql` committed |
| `Regelleistung:ProductionActivationEnabled` Konfigurations-Slot | Existiert seit RM-M4-03-D auf `RegelleistungOptions` (Default `false`); IConfiguration-Binding-Carve-out folgt im Host-Wave | ✅ |
...
- **Security-Profile:** **heute placeholder** — der
  ...
  Security-Profile-Check existiert (siehe F-12 in §9). In dieser
  Konstellation gilt der Slice als bewusst **nicht produktiv schaltbar**;
  produktive Aktivierungsfähigkeit ist erst nach F-12 zu verifizieren.
...
- aFRR-positiv- und aFRR-negativ-Profiltests dispatchen die
  ...
  und kann damit nicht als produktiv schaltbar gelten; diese beiden
  Profiltests dürfen deshalb auf den Test-Durchfluss mit
...
Slice abgeschlossen über fünf Sub-Slice-Commits + ein Review-Pass:

- **RM-M4-03-A** (092ab29) — Domain-Modell, Time-Validator,
  Debounce-Primitive, `RegelleistungOptions` mit Master-DoD-Defaults.
- **RM-M4-03-B** (2f9e9b3) — Persistenter Idempotenz-Tracker
  (`IActivationDedupeStore` + InMemory + `DapperActivationDedupeStore`)
  mit Tracker-Load-Fail-Closed-Pfaden; `0002_regelleistung_activations.sql`-
  Migration als idempotente Delta + regenerierter `0001_initial.sql`-
  Snapshot — erster realer Konsument von **RM-M3-FUP-01 ✅**.
- **RM-M4-03-C** (360a762) — `ActivationValidator`-Pipeline
  (Schema → UTC-Time → TimebaseDegraded → Dedupe; Order-Pin gegen
  Replay-Hit-bypass).
- **RM-M4-03-D** (73d3251) — Use-Case + Production-Gate (D-03 fail-
  closed bis F-12) + mFRR-fail-closed (D-05) + `IActivationDispatchSource`
  (D-09 c, additiv zu `ScheduleFollowingDispatchOptimizer`) +
  `GET /health/regelleistung`.
- **RM-M4-03-D Review-Pass** (81afb2c) — `product-trust-not-established`
  als spezifischer Reason-Code, `ILogger`-Audit-Trail (Event 4100),
  Test-Cleanup.
- **RM-M4-03-E** — Profiltests (aFRR positiv/negativ + mFRR-modelable),
  Race-Tiebreak-Tests, TimebaseDebounce-Pipeline-Pin, Postgres-Restart-
  Replay + Persistenz-Determinismus.

`make gates` und `make test-integration` bleiben grün; `make schema-
drift-check` ist grün nach Commit (Sub-Slice B). `note-RM-M3-followups.md`
Item 5 (FUP-01) ist auf ✅ — RM-M4-03-B ist das Pattern-Beispiel für
zukünftige Migrationen.

Folgearbeiten F-08..F-12 sind in §9 dokumentiert; F-12 (Security-
Profile als Cross-Adapter-Health-Signal) ist die einzige sperrende
Bedingung dafür, dass der Production-Code-Pfad mit
`ProductionActivationEnabled=true` produktiv schalten kann — solange
der Default-Provider `security-profile-enforcement-not-wired`
fail-closed liefert, ist der Slice **bewusst nicht produktiv
schaltbar**.
