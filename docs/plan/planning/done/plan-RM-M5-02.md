...
[`plan-RM-M5-01.md`](plan-RM-M5-01.md) (Contract-
...
| ✅ | RM-M5-02-B | QP-Solver-Backend `LocalOsqpMpcSolver` + erster Solver-Pin — **~800-1200 LOC** | Konkreter `IMpcModelSolver`-Adapter `LocalOsqpMpcSolver` im Sub-Namespace `BatteryEms.Adapters.Optimization.Mpc.Local`. `Strict`-Disziplin ist gepinnt (`warm_start=false`, `scaling=0`, `polish=false`, `threads=1`) und der dedizierte Determinismus-Pin bestätigt byte-identische `MpcTrajectory.Points` für identische Inputs. `BessHostOptions.MpcBackend = "local_osqp"` aktiviert den Adapter über `AddBessLocalOsqpMpcSolver`; Default `null` bleibt ohne MPC-Registration. **Negativ-Pin erfüllt:** `MpcBackend = "optimization_core"` oder `"bi_modal"` wirft `mpc-backend-not-implemented` und hält die F-M5-12-Namen frei. **Geliefert 2026-05-12:** Adapter-Dateien unter `src/adapters/driven/BatteryEms.Adapters.Optimization/Mpc/Local/`, Host-Slot `BessHostOptions.MpcBackend`, Host-Kompositionsgate, 12 neue Pins über `LocalOsqpMpcSolverRoundtripTests`, `LocalOsqpMpcSolverNegativeTests` und `MpcBackendCompositionTests` (Roundtrip, Strict-Flags, Determinismus, Orchestrator-Wire, DI-Registration, Time-Limit, Infeasible, Modell-invalid, Status-Mapping, reservierte Backend-Namen). Verifikation: `make test` und `make arch-check` grün. TestSidecar-MPC-Stubs entfallen weiter aus dem M5-02-Scope (siehe ADR 0006 — Sidecar-Linie ist F-M5-12). |
| ✅ | RM-M5-02-C | Kalman-Filter + Robustheits-Pfade + Fallback-Erweiterung + Monotonic-Clock — **~900-1100 LOC** | **Geliefert 2026-05-12:** `DefaultLinearKalmanFilter` in `BatteryEms.Application.Mpc` mit Predict-Step (`x_pred = A * x_old + B * u`, `P_pred = A * P_old * A^T + Q`) und Update-Step (`K = P_pred * C^T * (C * P_pred * C^T + R)^{-1}`, `x_new = x_pred + K * (y - C * x_pred)`, `P_new = (I - K * C) * P_pred`). Robustheits-Pfade sind gepinnt: Missing-Measurement-Skip mit persistentem Counter bis `mpc-state-stale-too-long`, unplausible SOC-/Temperaturwerte gegen Asset-Bounds mit `mpc-state-non-physical`, Covariance-Divergence mit `mpc-covariance-diverged`. `IMpcStateEstimator` trägt `EstimatorVariant`; `DefaultMpcDispatchOrchestrator` stempelt `estimator_variant`. Fallback-Erweiterung: `IFallbackMpcOptimizer`-Driven-Port, `MpcOptions.SolverFallbackProfile` (`MpcFallbackOptions` mit Solver-Profil + reduziertem Horizon) und `LocalOsqpFallbackMpcOptimizer`; `AddBessLocalOsqpMpcSolver` registriert den Kalman-Estimator und den lokalen Fallback-Port. Sidecar-basierte Fallback-Linie bleibt F-M5-12. **MonotonicAnchoredClock** wurde als `BatteryEms.Infrastructure.Time.MonotonicAnchoredClock` geliefert: Wall-Clock-Anchor + Stopwatch-/Timestamp-Offset, toleranzgeprüfter Resync, Reject bei großem Rücksprung; Composition-Root-Produktionsgates bleiben in Sub-Slice D. Neue Pins über `DefaultLinearKalmanFilterTests`, `MonotonicAnchoredClockTests`, Fallback-Wire-/Fallback-Profil-Pins und aktualisierte Orchestrator-/Composition-Pins. Verifikation: `make test` und `make arch-check` grün. |
| ✅ | RM-M5-02-D | Reproduzierbarkeitsvertrag + Replay-Hooks + Worker-Wiring + Retention + Production-Boot-Gates + Quality-Doku + Master-Plan-Closure — **~900-1100 LOC** | **Geliefert 2026-05-12:** `MpcRun` + `IMpcRunRepository`/`InMemoryMpcRunRepository`, `DapperMpcRunRepository` und Migration `0004_mpc_runs.sql` mit acht Identity-Spalten, `mpc_request_id`-PK, `numerik_stamp_json`, `p0_frobenius_display`, `deterministic_mode`, Terminal-/Trajektor-Feldern und drei Indizes. `MpcRunIdentity` bildet das vollständige 8-Feld-Tuple (`asset_id`, Tick, SampleTime, ModelVersion, EstimatorVariant, SolverConfigHash, EstimatorConfigHash, RandomSeed) als SHA-256-Request-ID; Default-Seed ist deterministisch, Operator-Seed-Override ist gepinnt. `MpcDispatchResult.Stamps` enthält Replay-Stempel inkl. `solver_config_hash`, `estimator_config_hash`, `numerik_stamp_json` und sichtbarer P0-Frobenius-Norm. `FallbackPlanValidator` prüft MPC-Stempel (`state_estimator_variant`, `estimator_config_hash`) und lässt `p0_frobenius_display` nur sichtbar, nicht identitätsbildend. Worker-Wiring ruft optional registrierte `IMpcDispatchOptimizer` pro Tick/Asset; der Default ohne `MpcBackend` bleibt ohne MPC-Step. Production-Gates: fehlender Fallback-Port ⇒ `mpc-production-without-fallback-pathway`, fehlende monotone Clock ⇒ `mpc-production-without-monotonic-clock`, reservierte Backends werfen zuerst `mpc-backend-not-implemented`. Neue Pins: Identity-Achsen-Serie, Cross-Run-Determinismus, InMemory-MpcRun-Replay/Retention, Fallback-MPC-Stamps, Worker-Counting-Stub und Composition-Gates. Quality-Doku und `make test-mpc-property` ergänzt; Master-Plan-Zeile RM-M5-02 flippt auf ✅. Verifikation: `make test` grün. |
...
2. **Sub-Slice B ✅ geliefert 2026-05-12**: QP-Solver-Backend `LocalOsqpMpcSolver` (D-02 /
...
3. **Sub-Slice C ✅ geliefert 2026-05-12**: Kalman-Filter + Robustheits-Pfade + Fallback-
