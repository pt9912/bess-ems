...
| ✅ | HIL-03 | Float-Word-Order konfigurierbar machen | `RegisterDecoder.Decode` und `Encode` switchen auf `mapping.WordOrder` und unterstützen `high_low` (M1-Default) plus `low_high`; unbekannte Werte werfen. Tests: Float32-Roundtrip + Int32-Roundtrip in beiden Reihenfolgen, Wort-Swap-Property, Reject auf unbekannte Order. HIL-01-Guards in `ModbusTelemetrySource` und `ModbusCommandSink` für `low_high` aufgehoben. |
| ✅ | HIL-04 | HIL-Modbus-Profil anlegen | `config/examples/adapters/modbus.hil-simulator.json` mit fünf FC04-Floats (active/reactive power MW→kW, voltage/frequency/current) und zwei FC03-Setpoints (P/Q), alle als `float32` mit `low_high` Word-Order. Schema-Roundtrip-Test (`Loads_HIL_modbus_profile_example`) pinnt die HIL-distinktiven Felder gegen Drift. Gerätepunkt-Basis (`display_name`, `unit`, `range`, `writable`, `write_cadence`, `auth_required`) trägt LH-DOM-005. |
| ✅ | HIL-05 | Optionalen Q-Setpoint schreiben | `ModbusCommandSink.WriteAsync` schreibt nach dem P-Setpoint zusätzlich `reactive_power_setpoint_kvar`, wenn das Mapping einen writable-Eintrag dafür enthält. Null-Q-Commands schreiben 0 kvar (verhindert dass das Gerät einen Stale-Q vom Vorgängercommand behält). Zwei neue Tests; Mappings ohne Q-Register bleiben unverändert (existierender VendorNeutralMapping-Test belegt das implizit). |
