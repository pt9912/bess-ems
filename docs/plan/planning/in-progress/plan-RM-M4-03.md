...
Idempotenz geprüft, und — bei akzeptierter Rezeption — als
Setpoint mit `ControlPriorityClass.RegelleistungsActivation`
(Rang 3) in den Dispatch-Pfad eingespeist. Safety-Limits (Rang 2)
bleiben vorrangig.
...
- **Security-Profile:** **heute placeholder** — der
  ...
  Production-Gate nicht produktiv grün werden — das ist
  intentional fail-closed bis ein realer Security-Profile-Check
  existiert (siehe F-12 in §9). **Pin-Test:** mit
  `ProductionActivationEnabled=true` läuft eine sonst-perfekte
  Aktivierung in `not-dispatch-relevant` mit Reason
