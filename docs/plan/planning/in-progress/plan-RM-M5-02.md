...
| ⬜ | RM-M5-02-B | QP-Solver-Backend `LocalOsqpMpcSolver` + erster Solver-Pin — **~800-1200 LOC** | Konkreter `IMpcModelSolver`-Adapter `LocalOsqpMpcSolver` per ADR 0006 (Backend-spezifischer Sub-Namespace im `BatteryEms.Adapters.Optimization.Mpc.*`-Baum — exakter Pfad ist Sub-Slice-B-Plan-Entscheidung). OSQP-Determinismus-Flags pin't der Adapter gemäß ADR 0006 §4 (`Strict`-Mode-Disziplin); konkreter Flag-Werte-Vertrag steht in der ADR, nicht hier. NuGet-Wrapper-Reuse vs. Eigenbau-P/Invoke entscheidet der Sub-Slice-B-Plan-Header. **OSQP-Determinismus-Validierungs-Pin** (ADR 0006 §4 Pflicht): empirische Bestätigung dass `Strict`-Flags byte-identische `MpcTrajectory.Points` liefern; bei Bruch wird Plan §5 D-04 in einer Folge-Iteration auf Toleranz-Mode (≤ 1e-12 relativ) angepasst (siehe ADR 0006 §4 Zwei-Stufen-Fallback). `BessHostOptions.MpcBackend = "local_osqp"` aktiviert den Adapter; **Negativ-Pin (Sub-Slice-B-Pflicht)**: Boot mit `MpcBackend = "optimization_core"` oder `"bi_modal"` ⇒ Startup-Fehler `mpc-backend-not-implemented` (reservierte F-M5-12-Werte); Composition-Root-Test bestätigt die Boot-Gate-Reihenfolge (Backend-Validation vor Production-Fallback-/Monotonic-Clock-Gates). 8+ Solver-Wire-Pins in neuer `LocalOsqpMpcSolverRoundtripTests` + `LocalOsqpMpcSolverNegativeTests` (Solver-Time-Limit, Infeasible, OSQP-Solver-Status-Mapping). Constraint-Property-Pins aus A werden gegen den echten OSQP-Solver erneut gefahren — die Property-Tests laufen jetzt nicht nur gegen den Stub sondern auch gegen die Production-Linie. TestSidecar-MPC-Stubs entfallen aus dem M5-02-Scope (siehe ADR 0006 — Sidecar-Linie ist F-M5-12). |
