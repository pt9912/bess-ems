...
- [ ] Sub-Slice 1 — `schema_version` Modbus/MQTT + Enforcement + Beispiel-Mappings.
- [ ] Sub-Slice 2 — Envelope-Schema (C#→Schema) + zwei-seitiger Check (Generat-Diff + Serializer-Round-trip).
- [ ] Sub-Slice 3 — Release-Asset-Bundle + Checksumme + CHANGELOG/`min_supported`.
- [ ] Sub-Slice 4 — `make field-contract-check` als Pflicht-Gate (make + CI).
- [ ] Sub-Slice 5 — `Bess__SnapshotMaxAge` konfigurierbar.
- [ ] Alle Akzeptanzkriterien erfüllt; `make gates` grün.
- [ ] ADR 0013 Status-Klausel auf `Accepted — §5.1 umgesetzt; §5.2–§5.4 offen` aktualisiert (bleibt Accepted; kanonischer String = Ziel).
