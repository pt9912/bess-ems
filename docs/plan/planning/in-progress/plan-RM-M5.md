...
| ⬜ | RM-M5-03 | Hochfrequente Telemetrie-Filterung im Native Core | Minimaler echter Slice: ca. 2-4 Arbeitstage fuer einen schmalen Native-Filtervertrag (Samplingrate, Einheiten, Fehlerverhalten), additive C-API-Erweiterung im `battery_control_core`, P/Invoke-Wiring im .NET-Adapter, Drift-/ungueltige-Eingaben-Pins und Einbindung in die bestehenden Native-Gates. .NET-Prechecks bleiben erhalten; Replay-/Numeriktests decken Drift und ungueltige Eingaben ab; invalider Filter-/MPC-State folgt der Fallback-Matrix. Produktionsnaher Ausbau mit Observability, breiter Replay-Matrix und Performance-SLOs bleibt Folgeumfang. |
