...
| ✅     | RM-M2-OP-05 | Adapter: LP-Solver-Adapter für `IScheduleOptimizer`                 | RM-M2-OP-02        | `OrToolsScheduleOptimizer` (GLOP-LP via `Google.OrTools` 9.11.4210, zentral gepinnt) im neuen `BatteryEms.Adapters.Optimization.OrTools`-Namespace; native Bindings restorieren im Debian-SDK-Image. Modell §Arbeitsmodell: `p_charge[t]`/`p_discharge[t]`/`soc[t]` mit SOC-Bounds aus `MinSocPercent`/`MaxSocPercent`/`Capacity`, SOC-Dynamik mit η_charge·Δt und Δt/η_discharge, Initial-SOC = `(min+max)/2` (oder `InitialSocPercent`-Override), freier Terminal-SOC, Objective `Σ price[t]·(p_charge−p_discharge)·Δt/1000` für `PriceUnit = "EUR/MWh"`. `OrToolsResultMapper` bildet alle 7 GLOP-Stati ab und erkennt Time-Limit-Hits über elapsed≥budget. `ScheduleSolverOptions` (`TimeLimit`/`GapTolerance`/`InitialSocPercent`/`DefaultMarketBidArea`) eager-validiert; DI per `services.AddBessScheduleSolver(configure)` überschreibt den NoOp last-wins (NoOp bleibt für API-only-Hosts). Schedule-Identity: existierenden Schedule lesen → MarketBidArea erben + Version+1; sonst Default-Bid-Area + Version 1. Pre-Flight-Failure-Pfade (`missing-prices`, `unsupported-price-unit:{unit}`, `initial-soc-out-of-bounds`) liefern `Failed`-Run mit klarer Reason und ohne Solver-Aufruf. 40 Adapter-Tests (Options-Validation, Mapper-Theory inkl. TimeLimit-Boundary, Pre-Flight-Failures, Konstruktor-Null-Guards, Cancellation, konstante Preise → end-of-horizon-Drain bei freiem Terminal-SOC, 2-Step-Arbitrage, Power-Limit-Saturierung, 30-min-Grid, ObjectiveBreakdown, Bid-Area-Inheritance, DI-Resolution NoOp↔OrTools, Configure-Pass-Through). `BuildNonSolutionResult` und der unreachable Mapper-Default sind `[ExcludeFromCodeCoverage]` markiert (LP unter M2-Inputs ist immer feasible an `p_charge=p_discharge=0`). `make lint` + `make test` (40/40 OP-Tests + alle anderen) + `make arch-check` (13/13) + `make coverage-gate` (Optimization 97.29 % line / 94.11 % branch ≥ 90 %) real grün. |
